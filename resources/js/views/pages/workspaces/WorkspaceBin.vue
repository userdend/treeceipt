...
const workspaceToRestore = ref(null)
...
const loadWorkspaces = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/workspaces/bin', {
      params: {
        page,
        itemsPerPage,
...
const confirmDelete = async () => {
  ...
  try {
    const response = await axios.delete(`/api/workspaces/data/${workspaceToDelete.value.id}/force`)
...
const openRestoreDialog = item => {
  workspaceToRestore.value = item
  isRestoreDialogOpen.value = true
}

const confirmRestore = async () => {
  if (!workspaceToRestore.value) return

  isRestoring.value = true
  try {
    const response = await axios.patch(`/api/workspaces/data/${workspaceToRestore.value.id}/restore`)

    workspaces.value = workspaces.value.filter(r => r.id !== workspaceToRestore.value.id)

    isRestoreDialogOpen.value = false
    workspaceToRestore.value = null
    success(response.data.message)
    loadWorkspaces()
  } catch (err) {
    error('Something went wrong. Please contact support.')
...
<template>
  ...
  >
    Are you sure you want to restore receipt <strong>{{ workspaceToRestore?.merchant }}</strong
